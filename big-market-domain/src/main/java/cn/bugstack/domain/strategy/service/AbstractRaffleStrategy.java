...
public abstract class AbstractRaffleStrategy implements IRaffleStrategy {
    ...
    public AbstractRaffleStrategy(IStrategyRepository repository, IStrategyDispatch strategyDispatch,
                                  DefaultChainFactory defaultLogicFactory,
    ...
    public RaffleAwardEntity performRaffle(RaffleFactorEntity raffleFactorEntity) {
        ...
        if (null == userId || null == strategyId) {
            log.error("抽奖参数异常 userId:{} strategyId:{}", userId, strategyId);
        ...
        if (!DefaultChainFactory.LogicModel.RULE_DEFAULT.getCode().equals(chainStrategyAwardVO.getLogicModel())) {
            // TODO：awardConfig 暂时为空。黑名单指定积分奖品，后续需要在库表中配置上对应的1积分值(或者其他)，并获取到。
     ...
     * @param strategyId:   策略ID
     * @param awardId:      奖品ID
     * @param expiredDate:  活动有效期限
     * @return StrategyAwardVO: 过滤结果
     * @author: qiuming
     */
    public abstract DefaultTreeFactory.StrategyAwardVO raffleLogicTree(String userId, Long strategyId, Integer awardId, Date expiredDate);
